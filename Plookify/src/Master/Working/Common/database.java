...
public class database
{
	...
	public ResultSet makeQuery(String query)//method to take a string as a query for database, returns resultset
		...
		catch (SQLException ex) {System.err.println(ex.getMessage()+"@mU");}
                finally
                {
                    try {conn.close();
                    System.out.println("connection closed at MU");} catch (SQLException ex)
        ...
        public boolean dupCheck(String search,String column,String table)//checks if there is a row match using what u searching for, the table and which column
        {
            ...
            try {
                ...
                while(check.next())
                    ...
                    try {conn.close();
                    System.out.println("connection closed at DC");} catch (SQLException ex)
                        {System.err.println(ex.getMessage());}
                    return count != 0;
                } 
        ...
        catch (SQLException ex) {System.out.println("resCount error!");}
